 ...
 * ```
 * var api = new API('corpid', 'secret', function (callback) {
 *   // 传入一个获取全局token的方法
 ...
 */
var API = function (corpid, corpsecret, getToken, saveToken, updateToken) {
  this.corpid = corpid;
  ...
  this.updateToken = updateToken || this.getAccessToken;
...
API.prototype.preRequest = function (method, args, retryed) {
  ...
  that.getToken(function (err, token) {
    ...
    if (token && !retryed) {
      ...
      that.updateToken(function (err, token) {
        // 如遇错误，通过回调函数传出
        if (err) {
          return callback(err);
        }
